...
do_download.parser = main_subparsers.add_parser('download-archive',
...
def do_download_tool(args):
    ...
    if len(sel) == 0:
        print('{}xbstrap{}: No tools to download'.format(colorama.Style.BRIGHT, colorama.Style.RESET_ALL))
        return

    if args.dry_run:
        for tool in sel:
            url = urllib.parse.urljoin(cfg.tool_archives_url + '/', tool.name + '.tar.gz')
            print('{}xbstrap{}: Will download tool {} from {}'.format(
                    colorama.Style.BRIGHT, colorama.Style.RESET_ALL,
                    tool.name, url))
        return
    ...
    for tool in sel:
        url = urllib.parse.urljoin(cfg.tool_archives_url + '/', tool.name + '.tar.gz')
        print('{}xbstrap{}: Downloading tool {} from {}'.format(
                colorama.Style.BRIGHT, colorama.Style.RESET_ALL,
                tool.name, url))
...
do_download_tool.parser = main_subparsers.add_parser('download-tool-archive',
        parents=[select_tools.parser])
do_download_tool.parser.add_argument('-n', '--dry-run', action='store_true',
